...
class AstNode VL_NOT_FINAL {
    ...
    // State variable used by V3Broken for consistency checking. The top bit of this is byte is
    // a flag, representing V3Broken is currently proceeding under this node. The bottom 7 bits
    // are a generation number. This is hot when --debug-checks so we access as a whole to
    // avoid bit field masking resulting in unnecessary read-modify-write ops.
...
public:
    ...
    // CONSTANTS
    // The following are relative dynamic costs (~ execution cycle count) of various
    // operations. They are used by V3InstCount to estimate the relative execution time of code
    // fragments.
    ...
    // Generated by 'astgen'. Dumps node-specific pointers and calls 'dumpJson()' of parent
    // class Note that we don't make it virtual as it would result in infinite recursion
...
public:
    ...
    // Predicate that returns true if the given 'nodep' might have a descendant of type
    // 'T_Node'. This is conservative and is used to speed up traversals. Note: specializations
    // for particular node types are provided below
...
public:
    // Given a callable 'f' that takes a single argument of some AstNode subtype 'T_Node',
    // traverse the tree rooted at this node, and call 'f' in pre-order on each node that is of
    // type 'T_Node'. The node passed to the callable 'f' can be removed or replaced, but other
    // editing of the iterated tree is not safe. Prefer 'foreach' over simple VNVisitor that
    // only needs to handle a single (or a few) node types, as it's easier to write, but more
    // importantly, the dispatch to the callable in 'foreach' should be completely predictable
    // by branch target caches in modern CPUs, while it is basically unpredictable for
    // VNVisitor.
    ...
    // return true if and only if all nodes of type 'T_Node' in the tree rooted at this node
    // satisfy the predicate 'p'. Returns true if no node of type 'T_Node' is present.
    // Traversal is performed in some arbitrary order and is terminated as soon as the result
    // can be determined.
    template <typename Callable>
